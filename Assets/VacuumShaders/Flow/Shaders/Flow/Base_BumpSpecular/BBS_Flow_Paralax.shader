Shader "VacuumShaders/Flow/Base_BumpSpecular/Flow_Paralax" {
	Properties {
		...
		_Emission("Flow Emission", Range(0, 1)) = 0
	...
	SubShader {
		...
		{
			float2 uv_BaseTex;
			//float2 uv_BaseBumpMap; //need more texture coords
		...
		void surf (Input IN, inout VacuumSurfaceOutput o)
		 {
			...
			IN.uv_BaseTex += offset;
			//IN.uv_BaseBumpMap += offset; //need more texture coords
			...
			o.Normal = UnpackNormal( tex2D(_BaseBumpMap, IN.uv_BaseTex) * invertFlow + finalB * flowMap.b ); 
